...
void loop(){
  ...
  //if there was no rain for the last 150 minutes then start sending the 10 files 
  //which are stored in the sd card
  if(noRainCounter == 10){
    lcd.backlight();

    while((noOfFile >= noOfFileGSM) && sendFlag == 1){

       //change int (noOfFileGSM) to string (sNoOfFileGSM)
       itoa(noOfFileGSM,sNoOfFileGSM,10);
  
       //concatinate .txt the end of the string file name
       strcat(sNoOfFileGSM, ".TXT");
        
       //read the file and put it on a buffer using the following function
       readFromSdCard();
...
int writeToSdCard() {
    ...
    if(nextFile){
      ...
      //close the file
      nextFile.close();
      //add one to the number of file written
      noOfFile++;
      ...
      //set the current push counter for a rain to 0 (zero)
      buttonPushCounter=0;
