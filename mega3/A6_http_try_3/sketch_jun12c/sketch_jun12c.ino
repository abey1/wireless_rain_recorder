...
void loop()
{
  ...
  updateSerial();
  mySerial.println("AT+CIPSTART=TCP,www.nrwlpms.com/sim900/a6_try_one.php?data=22,80"); //We connect to the server then the server will send back former data
  updateSerial();
  updateSerial();
  delay(2000);
  updateSerial();
  ...
  updateSerial();
  mySerial.print("123");                            //We send data to the server
  updateSerial();
  mySerial.write(26);                                //Termination symbol
  updateSerial();
  mySerial.println("AT+CDNSGIP=www.nrwlpms.com/sim900/a6_try_one.php?data=44");       //We query the IP address of given domain name
  updateSerial();
